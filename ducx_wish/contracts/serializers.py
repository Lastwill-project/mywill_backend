...
class TokenProtectorSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, contract_details):
        ...
        if contract_details.ducx_contract:
            if contract_details.ducx_contract.compiler_version:
                res['ducx_contract']['compiler_version'] = '.'.join(
                    res['ducx_contract']['compiler_version'].split('.')[:4])
...
class ContractDetailsNeoSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, contract_details):
        ...
        if not contract_details:
            print('*' * 50, contract_details.id, flush=True)
        # res['ducx_contract'] = EthContractSerializer().to_representation(contract_details.ducx_contract)
        return res
...
class ContractDetailsSTOSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, contract_details):
        ...
        if not contract_details:
            print('*' * 50, contract_details.id, flush=True)
        res['ride_contract'] = DUCXContractSerializer().to_representation(contract_details.ride_contract)
        return res
